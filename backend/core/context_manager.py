...
from backend.core.utils.tokens import estimate_tokens, estimate_message_list_tokens, select_messages_within_budget, truncate_to_token_limit

logger = logging.getLogger(__name__)

# Overhead calibration notes:
# SYSTEM_PROMPT_OVERHEAD = 50: accounts for message role tokens and
#   system-level framing that tokenizers add around the system prompt.
# TURN_OVERHEAD = 8: accounts for the per-message role/formatting token
#   overhead added by most LLM providers.
# These were calibrated for common GPT/Claude tokenizers and should be
# reviewed when adding new model families.
...
class ContextManager:
    ...
    def build_context(
        ...
        # Reserve: user message, max response tokens, and estimated overhead
        # The 50-token reserve accounts for system framing/metadata tokens
        ...
        if summary:
            ...
            if not summary_stripped:
                ...
                summary_tokens = estimate_tokens(summary_stripped, model)
                if summary_tokens > available:
                    summary = truncate_to_token_limit(summary_stripped, available, model)
        ...
        if relevant:
            ...
            if relevant_tokens > available:
                # Build kept list efficiently with deque to avoid O(n²) insert(0)
                keep = deque()
